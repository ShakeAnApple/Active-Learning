...
public class NusmvConverter {
    ...
    public static void saveInNusmvFormat(Automaton automaton, String path){
        try (Writer writer = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(path), "utf-8"))) {
            ...
            // inputs
            for (AbstractVariableInfo var : automaton.getInputVariableInfos()) {
            ...
            List<AbstractVariableInfo> intervalVars = automaton.getOutputVariableInfos().stream()
...
//
            for (State st : automaton.getStates()) {
                HashSet<Transition> transitionsFromSt = automaton.getTransitionsByFrom(st);
            ...
            for (AbstractVariableInfo var : automaton.getOutputVariableInfos()) {
                ...
                for (int i = 0; i < var.getPossibleValues().size(); i++) {
                    ...
                    for (State st : automaton.getStates()) {
                        RequestResult stateValue = st.getStateValue().getRequestResults().stream().filter(r -> r.getOrder() == 0).findFirst().get();
                        if (isIntervalValue) {
                            IntervalValueHolder i1 = (IntervalValueHolder) stateValue.getResponse().getVariableValueByName(var.getName()).getValue();
...
//                            }
                            ...
                            if (stateValue.getResponse().getVariableValueByName(var.getName()).getValue().equals(var.getPossibleValues().get(i))) {
                                curValStates.add(st);
                            }
                        }
                    }
